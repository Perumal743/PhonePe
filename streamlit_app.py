...
engine = create_engine("mysql+pymysql://root:Akash73@localhost:3306/phonepe")
...
if state_mode == "Statewise":
    States = sorted(agg_transaction["State"].unique().tolist())
    state_filter = st.sidebar.selectbox("Select State", States)

if year_mode == "Yearwise":
    Years = sorted(agg_transaction["Year"].unique().tolist())
    year_filter = st.sidebar.selectbox("Select Year", Years)


filtered_data = agg_transaction.copy()
if state_filter:
...
if selected_case == "Insurance Transactions Analysis":
    ...
    if "Transaction_amount" in agg_transaction.columns and "Insurance_amount" in agg_insurance.columns:
        trans_state = agg_transaction.groupby("State")["Transaction_amount"].sum().reset_index()
        ins_state = agg_insurance.groupby("State")["Insurance_amount"].sum().reset_index()
        merged = pd.merge(trans_state, ins_state, on="State", how="inner").sort_values("Transaction_amount", ascending=False)
        merged["Difference"] = merged["Transaction_amount"] - merged["Insurance_amount"]
        top10 = merged.nlargest(10, "Transaction_amount")
        ...
        width = 0.4
        x = range(len(merged))
        ax.bar(x, merged["Transaction_amount"], width, label="Transactions", color="skyblue")
        ...
        fig, ax = plt.subplots(figsize=(10, 5))
        sns.barplot(data=top10, x="State", y="Transaction_amount", palette="crest", ax=ax)
        plt.xticks(rotation=45)
        plt.tight_layout()
        st.pyplot(fig)
    ...
    txn_data = filtered_data.groupby("State")["Transaction_amount"].sum().reset_index()
    top10 = txn_data.nlargest(10, "Transaction_amount")

    st.dataframe(txn_data)

    fig, ax = plt.subplots(figsize=(12, 6))
    sns.barplot(data=txn_data, x="State", y="Transaction_amount", palette="coolwarm", ax=ax)
    plt.xticks(rotation=45)
    plt.tight_layout()
    st.pyplot(fig)
    ...
    st.write("### 🔝 Top 10 States - Transaction Amount")
    fig, ax = plt.subplots(figsize=(10, 5))
    sns.barplot(data=top10, x="State", y="Transaction_amount", palette="rocket", ax=ax)
    plt.xticks(rotation=45)
    plt.tight_layout()
    st.pyplot(fig)
